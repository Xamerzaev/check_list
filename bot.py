...
async def handle_successful_payment(message: Message):
    ...
    await update_profile_status_payment(message.from_user.id,
                                  provider_payment_charge_id)
    if message.successful_payment.total_amount == PRICES_IN_RATE[0]*100:
        await update_subscribe_period(message.from_user.id, DAYS_IN_RATE[0])
        await update_end_date(message.from_user.id, DAYS_IN_RATE[0])
        await update_profile_status(message.from_user.id, 4)
        await create_new_room(message)

    elif message.successful_payment.total_amount == PRICES_IN_RATE[1]*100:
        await update_subscribe_period(message.from_user.id, DAYS_IN_RATE[1])
        await update_end_date(message.from_user.id, DAYS_IN_RATE[1])
        await update_profile_status(message.from_user.id, 5)
    elif message.successful_payment.total_amount == PRICES_IN_RATE[2]*100:
        await update_subscribe_period(message.from_user.id, DAYS_IN_RATE[2])
        await update_end_date(message.from_user.id, DAYS_IN_RATE[2])
        await update_profile_status(message.from_user.id, 6)
